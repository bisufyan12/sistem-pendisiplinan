...
function initDropdowns() {
  const listKelas = [...new Set(DATA_SISWA.map(s => s.kelas))].sort();
  const indKelasOpt = document.getElementById('indKelas');
  indKelasOpt.innerHTML = '<option value="">-- Pilih Kelas --</option>';
  listKelas.forEach(k => {
    indKelasOpt.innerHTML += `<option value="${k}">${k}</option>`;
  });

  const indPel = document.getElementById('indPelanggaran');
  const kolPel = document.getElementById('kolPelanggaran');
  let pelOptions = '<option value="">-- Pilih Pelanggaran --</option>';
  DATA_PELANGGARAN.forEach(p => {
    pelOptions += `<option value="${p}">${p}</option>`;
  });
  indPel.innerHTML = pelOptions;
  kolPel.innerHTML = pelOptions;
}
...
function handleLogout() {
  ...
  // 2. Sembunyikan aplikasi utama, tampilkan halaman login
  document.getElementById('mainApp').classList.add('hidden');
  document.getElementById('pageLogin').classList.remove('hidden');

  // 3. Riset Form Login & Form Input
  const loginForm = document.getElementById('loginForm');
  if (loginForm) loginForm.reset();

  const formInd = document.getElementById('formIndividu');
  if (formInd) formInd.reset();

  const formKol = document.getElementById('formKolektif');
  if (formKol) formKol.reset();

  // 4. Reset teks profil di header
  document.getElementById('userPelapor').innerText = '-';
  document.getElementById('infoNamaPelapor').innerText = '-';
  document.getElementById('userRoleBadge').innerText = '-';
}


// NAVIGASI PENAMPILAN HALAMAN
function showSection(sec) {
  // Jika bukan Pembina mencoba akses Dashboard Admin, alihkan otomatis ke Form Pelaporan
  if (sec === 'dashboard' && (!currentUser || !currentUser.isPembina)) {
    sec = 'form';
  }

  ['sectionForm', 'sectionRekap', 'sectionDashboard'].forEach(s => {
    document.getElementById(s).classList.add('hidden');
  });

  ['navFormBtn', 'navRekapBtn', 'navDashboardBtn'].forEach(b => {
  ...
  if (sec === 'form') {
    ...
    document.getElementById('navFormBtn').className = "px-5 py-2.5 bg-slate-900 text-amber-400 rounded-xl text-sm font-bold shadow-sm transition whitespace-nowrap";
  } else if (sec === 'rekap') {
    document.getElementById('sectionRekap').classList.remove('hidden');
    document.getElementById('navRekapBtn').className = "px-5 py-2.5 bg-slate-900 text-amber-400 rounded-xl text-sm font-bold shadow-sm transition whitespace-nowrap";
    renderRekapSiswa();
  } else if (sec === 'dashboard' && currentUser.isPembina) {
    document.getElementById('sectionDashboard').classList.remove('hidden');
    document.getElementById('navDashboardBtn').className = "px-5 py-2.5 bg-slate-900 text-amber-400 rounded-xl text-sm font-bold shadow-sm transition whitespace-nowrap";
    renderDashboardTable();
  }
}
...
function switchTab(tab) {
  if (tab === 'individu') {
    ...
    document.getElementById('formKolektif').classList.add('hidden');
    document.getElementById('tabIndividuBtn').className = "flex-1 py-2.5 text-center font-bold text-slate-900 border-b-2 border-amber-400 transition";
    document.getElementById('tabKolektifBtn').className = "flex-1 py-2.5 text-center font-semibold text-slate-400 border-b-2 border-transparent hover:text-slate-600 transition";
  } else {
    document.getElementById('formIndividu').classList.add('hidden');
    document.getElementById('formKolektif').classList.remove('hidden');
    document.getElementById('tabKolektifBtn').className = "flex-1 py-2.5 text-center font-bold text-slate-900 border-b-2 border-amber-400 transition";
    document.getElementById('tabIndividuBtn').className = "flex-1 py-2.5 text-center font-semibold text-slate-400 border-b-2 border-transparent hover:text-slate-600 transition";
  }
}
...
function filterSiswaIndividu() {
  ...
  const indSiswa = document.getElementById('indSiswa');
  
  if (!kls) {
    indSiswa.disabled = true;
    indSiswa.className = "w-full p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-400 focus:border-slate-800 bg-slate-100 text-slate-500 outline-none text-sm transition";
    indSiswa.innerHTML = '<option value="">-- Pilih Kelas Terlebih Dahulu --</option>';
    return;
  }

  const filtered = DATA_SISWA.filter(s => s.kelas === kls);
  indSiswa.disabled = false;
  indSiswa.className = "w-full p-3 bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-400 focus:border-slate-800 outline-none text-sm transition";
  
  let opts = '<option value="">-- Pilih Siswa --</option>';
  filtered.forEach(s => {
    opts += `<option value="${s.nama}">${s.nama}</option>`;
  });
  indSiswa.innerHTML = opts;
}

function liveSearchSiswa() {
  const q = document.getElementById('kolSearch').value.toLowerCase().trim();
  ...
  const match = DATA_SISWA.filter(s => s.nama.toLowerCase().includes(q) || s.kelas.toLowerCase().includes(q)).slice(0, 5);
  
  if (match.length === 0) {
    resContainer.innerHTML = '<div class="p-3 text-xs text-slate-400">Siswa tidak ditemukan</div>';
  } else {
    resContainer.innerHTML = match.map(s => `
      <div onclick="tambahSiswaKolektif('${s.nama}', '${s.kelas}')" class="p-3 hover:bg-amber-50 cursor-pointer text-xs font-semibold flex justify-between">
        <span>${s.nama}</span>
        <span class="text-slate-400 font-normal">${s.kelas}</span>
      </div>
    `).join('');
  }
...
function tambahSiswaKolektif(nama, kelas) {
  if (!selectedKolektifSiswa.some(s => s.nama === nama)) {
    selectedKolektifSiswa.push({ nama, kelas, ket: '' });
...
function hapusSiswaKolektif(idx) {
  selectedKolektifSiswa.splice(idx, 1);
  renderSelectedKolektif();
}

function updateKetKolektif(idx, val) {
  selectedKolektifSiswa[idx].ket = val;
}

function renderSelectedKolektif() {
  ...
  container.innerHTML = selectedKolektifSiswa.map((s, idx) => `
    <div class="flex items-center gap-2 bg-white p-2.5 rounded-lg border border-slate-200">
      <div class="flex-1 text-xs">
        <span class="font-bold text-slate-800">${s.nama}</span> <span class="text-slate-400">(${s.kelas})</span>
      </div>
      <input type="text" placeholder="Keterangan..." value="${s.ket}" oninput="updateKetKolektif(${idx}, this.value)" class="p-1.5 bg-slate-50 border border-slate-200 rounded text-xs outline-none focus:bg-white w-1/3">
      <button type="button" onclick="hapusSiswaKolektif(${idx})" class="text-rose-500 hover:text-rose-700 font-bold text-xs px-2 py-1">Hapus</button>
    </div>
  `).join('');
}
...
function renderRekapSiswa() {
  const container = document.getElementById('rekapSiswaBody'); // Sesuaikan ID container/tabel rekap Anda
  if (!container) return;
...
function renderDashboardTable() {
  ...
  document.getElementById('statIndividu').innerText = listLaporan.filter(l => l.tipe === 'Individu').length;
  document.getElementById('statKolektif').innerText = listLaporan.filter(l => l.tipe === 'Kolektif').length;

  const tbody = document.getElementById('rekapTableBody');
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" class="p-4 text-center text-xs text-slate-400">Tidak ada laporan ditemukan.</td></tr>';
    return;
  }
...
function bukaModalPanggilan(nama, kelas, total) {
  ...
  document.getElementById('modalNamaSiswa').innerText = `Nama Siswa: ${nama}`;
  document.getElementById('modalKelasSiswa').innerText = `Kelas: ${kelas}`;
...
function closeModalPanggilan() {
  document.getElementById('modalPanggilan').classList.add('hidden');
  selectedSiswaPanggilan = null;
}

function prosesCetakPanggilan() {
  if (!selectedSiswaPanggilan) return;

  const waktu = document.getElementById('panggilanWaktu').value;
  const ruang = document.getElementById('panggilanRuang').value;
  const catatan = document.getElementById('panggilanCatatan').value;

  if (!waktu) {
    alert('Harap tentukan tanggal dan waktu pemanggilan!');
    return;
  }
